...
def run_easybrowser_openai_web_login_flow(
    *,
    ...
    provider_hint = _env_text("OPENAI_WEB_LOGIN_BROWSER_PROVIDER_HINT", _env_text("OPENAI_COMMUNITY_BROWSER_PROVIDER_HINT", DEFAULT_BROWSER_PROVIDER_HINT))
    browser_backend = _env_text("OPENAI_WEB_LOGIN_BROWSER_BACKEND", _env_text("OPENAI_COMMUNITY_BROWSER_BACKEND", DEFAULT_BROWSER_BACKEND))
    acquire_payload: dict[str, Any] = {
        "request_id": f"openai-web-login-acquire-{uuid.uuid4().hex[:12]}",
        "mode": "direct",
        ...
        "metadata": {
            "caller": "easyprotocol-openai-web-login",
        },
    }
    ...
    try:
        flow_payload = {
            "request_id": f"openai-web-login-flow-{uuid.uuid4().hex[:12]}",
            "flow_type": "login",
